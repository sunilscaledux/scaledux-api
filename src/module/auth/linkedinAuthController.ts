...
export async function linkedinCallback(req: Request, res: Response) {
  try {
    ...
    let tokenResponse;
    try {
      tokenResponse = await axios.post<LinkedInTokenResponse>(
    ...
    try {
      ...
      // Get localized names
      firstName =
        (Object.values(linkedinUser.firstName?.localized || {})[0] as string) ||
        "";
      lastName =
        (Object.values(linkedinUser.lastName?.localized || {})[0] as string) ||
        "";
    }
